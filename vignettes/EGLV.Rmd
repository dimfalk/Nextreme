...
devtools::install_github("DWA-A-531/Nextreme", build_vignettes = TRUE)
```
...
```{r}
# 5-minutely sums 1991-2020; caution: 83 MB!
fname <- "n2166_01011991-01012021_5minutenwerte_sum.csv"
...
unique(diff(prec.df$Datum))

# Ueberpruefung des Wertebereichs der RH
range(prec.df$RH, na.rm = FALSE)
```
...
maxIntSerie = jaehrliche_maxSerie(prec.df, 
                                           Dauern = dur, 
                                           DSDmin = 240, 
                                           Intervall = 5,
                                           SerieTyp = "INT")
...
Die Variable *maxIntSerie* ist eine Tabelle mit der maximalen Intensitaet (mm/h), die bei verschiedenen Dauern (in Spalten) und Jahren (in Zeilen) ermittelt wurde.
...
```{r}
maxIntSerie <- Intervallkorrektur(maxIntSerie, Intervall = 5)
...
```{r}
# WechselDatum = as.Date("1991-01-01", format = "%Y-%m-%d")
# 
# maxSerie_korrigiert = Sprung_Korrektur(Goerlitz_maxIntSerie, WechselDatum)
# 
# # Die Differenz vor und nach der Funktion `Sprung_Korrektur()` ueberpruefen.
# round(Goerlitz_maxSerie_korrigiert - Goerlitz_maxIntSerie, 0) |> head(10)
# 
# all.equal(Goerlitz_maxIntSerie, Goerlitz_maxSerie_korrigiert)
```
...
```{r}
N_pars      = Parameter_Schaetzung(maxIntSerie, 
                                   Dauern = dur, 
                                   methGEV = "GEV", 
                                   formTyp = "FIX", 
                                   Gamma = -0.1,
...
```{r}
KI      = Unsicherheit_Schaetzung(maxIntSerie,
                                  Tn = tn,
                                  Dauern = dur,
                                  methGEV = "GEV",
                                  formTyp = "FIX",
...
plot(dur, H_quas["100", ], type = "l", lwd = 2, lty = 2, log = "xy",
     ...
     ylab = "hN [mm]", xlab = "Dauer [min]", main = "Station 2166")
polygon(c(dur, rev(dur)), c(HN_KI$`2.5%`["100", ],
...
# Option 2 
barplot(as.matrix(KI),col = Tn_Farbe, beside = TRUE, ylim = c(0,50), ylab = "KI [%]",
        xlab = "Dauer [min]", main = "Station 2166")
legend("top", legend = c(2, 5, 10, 20, 50, 100), fill = Tn_Farbe, cex = 0.8, 
       title = "Tn", horiz = TRUE, bty = "n")

# Option 3 
Dauer_Farbe = hcl.colors(dim(KI)[2], "blues", rev = TRUE)

barplot(as.matrix(t(KI)), col = Dauer_Farbe, beside = TRUE, ylim = c(0, 60), 
        ylab = "KI [%]", xlab = "Tn", main = "Station 2166")
...
coords <- c(389921.58, 5701745.339)
