...
Kostra2020_hN_Schaetzung <- function(Standorte,
  ...
  else if(any(is.na(Standorte) == TRUE)) stop("Das Standorte Input enthaelt mindestens einen fehlenden Werte! Entfernen Sie die fehlenden Wert")
  ...
  else if(any(is.na(Dauern) == TRUE)) stop("Das Dauern Input hat fehlende Werte! Bitte entfernen Sie diese.")
  ...
  else if(any(is.na(Tn) == TRUE)) stop("Das Tn Input hat fehlende Werte! Bitte entfernen Sie diese.")
  ...
  Kostra_HN <- data.frame(ID = Standorte$Stations_id,
  ...
  Kostra_UC = data.frame(ID = Standorte$Stations_id,
                         geoBreite = Standorte$geoBreite,
                         geoLaenge = Standorte$geoLaenge,
                         Kostra_UC_perDauer)
  ...
  fnames <- c(list.files(path = Temp_Pfad, pattern = "^Hn_KOSTRA-DWD-2020_.*asc$"),
