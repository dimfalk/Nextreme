#' Berechnung der jährlichen maximalen Serie
#'
#' @description
#' Berechnung der jährlichen maximalen Serie (basierend auf dem Kalenderjahr) aus
#' einer Niederschlagszeitreihe und vorgegebenen Dauerstufen.
#'
#' @param Regendaten data.frame. Gemessene Niederschlagshöhen in äquidistanten
#'     Intervallen, vorzugsweise 5-minütlich. Format: Datum (als POSIXct) in erster
#'     Spalte, Niederschlagshöhe (als numeric) in zweiter Spalte namens `"RH"`.
#'     Fehlende Werte sind als `NA` anzugeben.
#' @param Dauern numeric. Dauerstufen, die für die Berechnung der jährlichen Serien
#'     herangezogen werden, in gleicher Einheit (entweder in Minuten oder Stunden)
#'     wie das Intervall von `Regendaten`.
#' @param Intervall numeric. Zeitintervall der Niederschlagsmessungen
#'     (entweder in Minuten oder Stunden).
#' @param DSDmin numeric. Mindesttrockendauer, die für die Unabhängigkeit der Extremwerte
#'     erforderlich ist, angegeben in der gleichen Einheit wie `Intervall`.
#' @param SerieTyp character. Typ der ausgegebenen jährlichen Serie, entweder als
#'     Volumen in mm pro Dauer (`"VOL"`) oder Intensität in mm/h (`"INT"`).
#' @param report (optional) character. Ordnerpfad, in dem die Informationen über die
#'     jährlichen Extremwerte gespeichert werden sollen.
#'
#' @details
#' Funktion zur Ermittlung der jährlichen Serie (auf der Grundlage des Kalenderjahres)
#' aus einer Regenzeitreihe und vorgegebenen Dauerstufen. Eine Mindesttrockendauer
#' wird verwendet, um unabhängige Regenereignisse am Anfange bzw. Ende eines Jahres
#' zu identifizieren.
#'
#' Für weitere Informationen siehe Kap. 5.2 des Arbeitsblattes DWA-A 531 (2025).
#'
#' @return data.frame. Jährliche maximale Serie (als Niederschlagshöhe in mm oder
#'     Niederschlagsintensität in mm/h). Anzahl der Zeilen entspricht den Jahren
#'     mit verfügbaren Daten, die Spalten bezeichnen die ausgewählten Dauerstufen.
#' @export
#'
#' @examples
#' \dontrun{
...
jaehrliche_maxSerie <- function(Regendaten,
  ...
  if (missing(Regendaten)) {
    stop("Das Regendaten Input ist nicht vorhanden! Bitte geben Sie einen data.frame der Regenzeitreihe mit 2 Spalten: Datum - Date() Typ und RH - numeric() Typ.")
  } else if (class(Regendaten) != "data.frame") {
    stop("Das Regendaten Input sollte als data.frame sein! Bitte geben Sie einen data.frame() der Regenzeitreihe mit 2 Spalten: Datum - Date() Typ und RH - numeric() Typ.")
  } else if (dim(Regendaten)[1] < 5) {
    stop("Das Regendaten Input enthaelt weniger als 5 Zeitschritte! Um Fehler zu vermeiden, sind mindestens 5 Zeitschritte erforderlich.")
  } # AP comment: this condition might not be meaningful, as results with only 5 measurements are likely not reliable.

  else if (dim(Regendaten)[2] != 2) {
    stop("Das Regendaten Input soll zwei Spalten mit Namen Datum und RH enthalten! Die Spalte 'Datum' soll die Datumsangaben zur Messung enthalten (im Format Date oder POSIXct). Die Spalte 'RH' soll die Regenhoehe mm/Intervalllaenge enthalten (im Format numeric).")
  } else if (any(c("Datum", "RH") %in% colnames(Regendaten) == FALSE) == TRUE) {
    stop("Das Regendaten Input soll zwei Spalten mit Namen Datum und RH enthalten! Die Spalte 'Datum' sollte die Datumsangaben zur Messung enthalten (im Format Date oder POSIXct). Die Spalte 'RH' sollte die Regenhoehe mm/Intervalllaenge enthalten (im Format numeric)")
  } else if (any(class(Regendaten$Datum) %in% c("Date", "POSIXct", "POSIXt") == FALSE) == TRUE) {
    stop("Die Spalte 'Datum' der Regendaten ist keines der akzeptierten Formate: Date, POSIXct oder POSIXt!")
  } else if (class(Regendaten$RH) != "numeric") {
    stop("Die Spalte 'RH' der Regendaten ist nicht numerisch!")
  } else if (any(Regendaten$RH < 0, na.rm = TRUE) == TRUE) stop("Die Spalte 'RH' der Regendaten enthaelt negative Werte!")
  ...
  } # AP comment: is it reasonable to define only one duration? Consider how this choice might influence the estimation of GEV parameters, particularly eta and theta.
  ...
  if (class(DSDmin) != "numeric") {
    stop("Das Input DSDmin sollte vom numerischen Typ sein!")
  } else if (length(DSDmin) != 1) {
    stop("Das Input DSDmin sollte nur 1 Element haben!")
  ...
  extremserie_jeDauer <- lapply(Dauern, function(dur) {
    ...
    extremserie_jeJahr <- lapply(Jahre_mit_Daten, function(Jahr) {
      if (dur > DSDmin) {
        ...
        entry <- Regendaten_Dur[which(Regendaten_Dur$YYYY == Jahr), ]
      }
  ...
  if (dir.exists(report) == TRUE) utils::write.table(Extra_Info, file = paste0(report, "Info_Jaehrliche_maxSerie.txt"), sep = ";", row.names = FALSE, col.names = TRUE, quote = FALSE)
  ...
  if (SerieTyp == "INT") {
    ...
    Endtabelle <- TabelleExtreme
  } else {
    stop(paste0("Gewuenschter Output [", SerieTyp, "] ist nicht gueltig! Bitte geben Sie [VOL] fuer die Hoehe der jaehrlichen Extremwerte oder [INT] fuer die Intensitaet der jaehrlichen Extremwerte an."))
  }
